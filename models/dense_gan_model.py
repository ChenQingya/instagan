...
class DenseGANModel(BaseModel):
	...
	def initialize(self, opt):
		...
		self.netG_A = networks_Densenet.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, growth_rate=32, block_config=(3, 3, 3, 3),num_init_features=32, bn_size=4, drop_rate=0, num_classes=1000, memory_efficient=False)	# opt.norm默认是'instance'
		self.netG_B = networks_Densenet.define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.netG, opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, growth_rate=32, block_config=(3, 3, 3, 3),num_init_features=32, bn_size=4, drop_rate=0, num_classes=1000, memory_efficient=False)
		if self.isTrain:
