...
class InstaGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm, not opt.no_dropout,
                                        opt.init_type, opt.init_gain, self.gpu_ids)  # opt.norm默认是'instance'
    ...
    def backward_G(self):  # 计算生成器的总loss并反向传播
        ...
        if self.forward_A:
            ...
            self.loss_G_A = self.criterionGAN.get_g_loss(self.netD_A, self.real_A_sng,
                                                         self.fake_B_mul)  # self.fake_B_mul用于计算loss，假的B的结果经过判别器，其结果（self.netD_A(self.fake_B_mul)）与希望的结果（True）之间的loss
            ...
            self.loss_idt_B = self.criterionIdt(self.netG_B(self.real_A_sng),
                                                self.real_A_sng.detach()) * lambda_A * lambda_idt  # self.real_A_sng.detach()
